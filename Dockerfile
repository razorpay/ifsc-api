...
# Layer 1: Install OS dependencies and the correct Bundler version (static, cached).
...
# Layer 2: Install the application's gems (cached unless Gemfile.build changes).
RUN bundle install --jobs=$(nproc) --retry 3

# Copy remaining build-specific files (cache-busting boundary)
COPY init.rb /app/
COPY data /app/data/
...
RUN apk --no-cache add dumb-init redis libstdc++ curl

COPY Gemfile Gemfile.lock /app/

# Install production gems.
RUN apk --no-cache add --virtual .build-deps g++ make && \
    gem install bundler -v 2.4.10 && \
